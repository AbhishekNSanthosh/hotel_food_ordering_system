...
function OrderStatusContent() {
  ...
  const fetchOrders = async (sid: string) => {
    if (!sid) return; // wait until session is ready
    try {
      const params = new URLSearchParams({ tableNumber, sessionId: sid });
      const res = await fetch(`/api/orders?${params}`);
      if (res.ok) {
        const data = await res.json();
        // Show all orders for this session (pending, preparing, ready, delivered, etc.)
  ...
  // Read session ID from sessionStorage — same key used when placing the order
  useEffect(() => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "Preparing":
        return <Clock className="w-6 h-6 text-orange-500 animate-spin-slow" />;
      case "Ready":
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-gray-200 px-6">
              ...
              <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">
                You haven&apos;t placed any orders in this session. Explore the
                menu and order something delicious!
              </p>
              <Link
                href={`/?table=${tableNumber}`}
            ...
            <div className="space-y-6">
              {orders.map((order) => (
                <div
                  key={order._id}
                  className="bg-white rounded-[2rem] border border-gray-200 overflow-hidden hover:border-primary/20 transition-all"
                >
                  <div className="p-6 md:p-8">
                    <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                      <div>
                        ...
                        <h3 className="text-sm font-mono font-bold text-gray-600 bg-gray-50 px-2 py-1 rounded">
                          #{order._id.slice(-8).toUpperCase()}
                        </h3>
                      </div>
                      <div
                        className={`px-4 py-2 rounded-2xl border-2 flex items-center gap-2 ${statusColors[order.status] || "bg-gray-50"}`}
                      >
                    ...
                    <div className="space-y-3 mb-6">
                      {order.items.map((item, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center p-3 rounded-2xl bg-gray-50 border border-gray-200"
                        >
                          ...
                          <span className="text-sm font-black text-gray-600 text-right">
                      ...
                      <div className="mt-6 bg-amber-50 border-2 border-amber-200 rounded-2xl p-4 flex items-start gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        ...
                        <div>
                          ...
                          <p className="text-amber-700 text-sm font-medium leading-tight mt-0.5">
                            {order.compensationNote ||
                              "Your order was delayed. Please enjoy a complimentary item on us!"}
                          </p>
                        </div>
                      </div>
                    )}
