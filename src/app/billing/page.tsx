...
export default function BillingDashboard() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const billingOrders = data.filter((o: Order) =>
  ...
  const calculatedSubtotal =
    itemSubtotal > 0 ? itemSubtotal : selectedOrder?.totalAmount || 0;

  const taxAmount = calculatedSubtotal * TAX_RATE;
  const serviceCharge = calculatedSubtotal * SERVICE_CHARGE_RATE;
  const finalTotal = Math.max(
    0,
    calculatedSubtotal + taxAmount + serviceCharge - appliedDiscount,
  );
            ...
            filteredOrders.map((order) => (
              ...
              >
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold text-slate-900 border border-slate-200 px-2 py-1 rounded bg-white">
                    ₹{order.totalAmount.toFixed(2)}
                  </span>
                </div>
                <div className="mt-2">
                  {order.paymentStatus === "Paid" ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-emerald-100 text-emerald-700">
                      <svg
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-amber-100 text-amber-700">
                      <svg
                        className="w-2.5 h-2.5"
                        fill="none"
                        viewBox="0 0 24 24"
        ...
        {selectedOrder ? (
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 flex flex-col print:shadow-none print:border-none print:p-0">
                ...
                <div className="text-center mb-8 pb-6 border-b-2 border-dashed border-slate-200">
                  ...
                  <p className="text-slate-500 text-sm">
                    45 Gourmet Avenue, Downtown
                  </p>
                  <p className="text-slate-500 text-sm">
                    Phone: +91 98765 12345 | Web: www.hoteldelish.com
                  </p>
                  <p className="text-slate-500 text-sm font-semibold mt-4 uppercase tracking-widest text-[10px]">
                    Tax Invoice / Receipt
                  </p>
                </div>
                ...
                <div className="flex justify-between text-sm mb-6 bg-slate-50 rounded-lg p-4 font-medium print:bg-transparent print:p-0 print:border-none border border-slate-100">
                  <div>
                    <p className="text-slate-500 mb-1">
                      Order ID:{" "}
                      <span className="text-slate-900 font-bold ml-1 font-mono">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b-2 border-slate-900 text-left">
                        <th className="pb-3 pt-2 font-bold uppercase text-[11px] tracking-wider text-slate-500">
                          Item Name
                        </th>
                        <th className="pb-3 pt-2 text-center font-bold uppercase text-[11px] tracking-wider text-slate-500">
                          Qty
                        </th>
                        <th className="pb-3 pt-2 text-right font-bold uppercase text-[11px] tracking-wider text-slate-500">
                          Rate
                        </th>
                        <th className="pb-3 pt-2 text-right font-bold uppercase text-[11px] tracking-wider text-slate-500">
                          Amount
                ...
                <div className="border-t-2 border-dashed border-slate-200 pt-6 space-y-3">
                  ...
                  <div className="hidden print:flex justify-between items-center pt-6 mt-6 border-t border-slate-200 font-bold uppercase tracking-wider text-sm">
                    ...
                    >
                      {selectedOrder.paymentStatus === "Paid"
                        ? `Paid Online${selectedOrder.razorpayPaymentId ? ` (${selectedOrder.razorpayPaymentId.slice(-8)})` : ""}`
                        : "Pending Settlement"}
                    </span>
                  </div>
                </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 print:hidden p-8 text-center bg-slate-50">
            ...
            <p className="text-slate-500 font-medium max-w-sm">
              Select an active order from the sidebar to generate a bill, apply
              discounts, and process payment.
            </p>
          </div>
        )}
      </main>
