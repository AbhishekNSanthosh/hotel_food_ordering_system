...
export default function CartSidebar({
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Sidebar */}
            <div className="relative flex h-full w-full max-w-md flex-col bg-background shadow-xl animate-in slide-in-from-right duration-300 border-l border-border">
                                    ...
                                    <div className="ml-4 flex flex-1 flex-col">
                                        <div>
                                            <div className="flex justify-between text-base font-medium text-foreground">
                                                ...
                                                <p className="ml-4 text-primary font-bold">${(item.menuItem.price * item.quantity).toFixed(2)}</p>
                                            </div>
                                            <p className="mt-1 text-sm text-muted-foreground">{item.menuItem.isVeg ? 'Veg' : 'Non-Veg'}</p>
                                        </div>
                                        <div className="flex flex-1 items-end justify-between text-sm">
                                            <div className="flex items-center space-x-2 border border-border rounded-md">
                                                <button
                                                    onClick={() => onUpdateQuantity(item.menuItem._id, Math.max(1, item.quantity - 1))}
                ...
                {cart.length > 0 && (
                    <div className="border-t border-border px-6 py-6 bg-muted/20">
                        ...
                        <div className="space-y-4 mb-4">
                            <div>
                                <label htmlFor="customerName" className="block text-sm font-medium text-muted-foreground">Name (Optional)</label>
                                <input
                                    type="text"
                                    id="customerName"
